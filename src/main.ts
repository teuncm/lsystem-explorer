...
showAxiom(axiom, grammar, theta, nIter);
...
function setupScene() {
    ...
    camera.position.set( 0, AXIOM_Y + CAM_MIN_DIST, CAM_OFFSET_Z );
    ...
    controls.panSpeed = 2;
    controls.target = new THREE.Vector3( 0, AXIOM_Y, 0 );
    controls.update();
...
function showAxiom(axiom: string, grammar: grammar, theta: number, nIter: number) {
    let lSystem = new LSystem(axiom, grammar, theta);
    lSystem.iterateN(nIter);
    ...
    let camDist = AXIOM_Y + CAM_PADDING*Math.max(camDistW, camDistH, CAM_MIN_DIST);
...
function addAxiom(lines2D: Line2D[]) {
